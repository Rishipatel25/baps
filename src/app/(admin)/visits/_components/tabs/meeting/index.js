...
const TabMeeting = ({ currentVisitId, tabStatus }) => {
  ...
  const getServicesTabList = () => {
    dispatch(
      getAllMeetingAction({ visitId: currentVisitId }, setServiceTabsAndData),
    );
  };
          ...
          <div className="d-flex service-tab-wrapper gap-2 mt-3">
            ...
            {tabList.map((tab, idx) => {
              return (
                <button
                  key={idx}
                  className={`border-raduis-none mb-0 ${
      ...
      {selectedTabData?.visitServiceModelList?.length
        ? selectedTabData?.visitServiceModelList?.map((tab, idx) => {
            ...
            tabContent.meetingWith =
              tab?.meetingPersonnel?.firstName +
              ' ' +
              tab?.meetingPersonnel?.lastName;
            return (
              <Accordian
                key={idx}
                ...
                cb={(action) => {
                  if (action === 'REMOVE') {
                    toast.success(
                      TOAST_SUCCESS.MEETING_TEMPLATE_DELETED.MESSAGE,
                      {
                        toastId: TOAST_SUCCESS.MEETING_TEMPLATE_DELETED.ID,
                      },
                    );
                    getServicesTabList();
                  } else {
